...
(function($) {
	...
	$.fn.qtip = function(options, blanket) {
		...
		return $(this).each(function() // Return original elements as per
				// jQuery guidelines
				{
						...
						if (typeof interfaces == 'object') {
							...
							// API call is not a BLANKET DESTROY command
								...
								for (i = 0; i < interfaces.length; i++) {
									...
									// Only call API if tooltip is rendered and
					...
					// No API commands, continue with qTip creation
						...
						// If prerendering is disabled, create tooltip on
						// showEvent
						if (config.content.prerender === false
							...
							config.show.when.target.bind(config.show.when.event
										...
										// Unbind show event and cache mouse
										// coords
										...
										// Render tooltip and start the event
										// sequence
										construct.call(api);
										api.options.show.when.target
		...
		$.extend(self, self.options.api, {
			show : function(event) {
				...
				function afterShow() {
					...
					// Prevent antialias from disappearing in IE7 by removing
					// filter attribute
					if ($.browser.msie)
					...
					switch (self.options.show.effect.type.toLowerCase()) {
						case 'fade' :
							self.elements.tooltip.fadeIn(
									self.options.show.effect.length, afterShow);
							break;
						case 'slide' :
					...
					switch (self.options.hide.effect.type.toLowerCase()) {
						case 'fade' :
							self.elements.tooltip.fadeOut(
									self.options.hide.effect.length, afterHide);
							break;
						case 'slide' :
			...
			updatePosition : function(event, animate) {
				...
				if (self.options.position.target !== 'mouse') {
					...
					if (self.options.position.target.get(0).nodeName
						...
						// into integers
						coords = self.options.position.target.attr('coords')
								.split(',');
						for (i = 0; i < coords.length; i++)
							coords[i] = parseInt(coords[i]);

						// Setup target position object
						mapName = self.options.position.target.parent('map')
								.attr('name');
						imagePos = $('img[usemap="#' + mapName + '"]:first')
								.offset();
						target.position = {
					...
					// Target is a regular HTML element, find position normally
						...
						// animated, retrieve position from newPosition data
				...
				ieAdjust = ($.browser.msie) ? 1 : 0; // And this is why I
				// hate IE...
				...
				if (self.options.position.adjust.screen === true)
					newPosition = screenAdjust.call(self, newPosition, target,
							tooltip);

				// If mouse is the target, prevent tooltip appearing directly
				// under the mouse
				if (self.options.position.target === 'mouse'
					...
					newPosition.left += (mouseAdjust.search(/right/i) !== -1)
							? -6
							: 6;
					newPosition.top += (mouseAdjust.search(/bottom/i) !== -1)
							? -6
							: 6;
				}
				...
				if (!self.elements.bgiframe && $.browser.msie
			...
			updateWidth : function(newWidth) {
				...
				hidden = self.elements.contentWrapper.siblings()
						.add(self.elements.tip).add(self.elements.button);
				...
				if (!newWidth) {
					...
					// No width is set, proceed with auto detection
						...
						// Set position and zoom to defaults to prevent IE
						// hasLayout bug
						if ($.browser.msie)
							self.elements.wrapper
									.add(self.elements.contentWrapper
											.children()).css({
												zoom : 'normal'
											});
						...
						// Make sure its within the maximum and minimum width
						// boundries
				...
				if (self.options.style.border.radius) {
					self.elements.tooltip.find('.qtip-betweenCorners').each(
							function(i) {
								$(this)
										.width(newWidth
				...
				if ($.browser.msie) {
					...
					// hasLayout bug)
					self.elements.wrapper.add(self.elements.contentWrapper
							.children()).css({
								zoom : '1'
							});
					...
					if (self.elements.bgiframe)
						self.elements.bgiframe.width(newWidth)
			...
			updateStyle : function(name) {
				...
				if (self.options.style.tip.corner !== false) {
					if ($('<canvas>').get(0).getContext) {
						...
						tip.attr('fillcolor', self.options.style.tip.color
										|| self.options.style.border.color);
					};
				};
				...
				if (self.options.style.border.radius > 0) {
					...
					if ($('<canvas>').get(0).getContext) {
						...
						self.elements.tooltip.find('.qtip-wrapper canvas')
								.each(function() {
									// Retrieve canvas context and clear
									...
									// Draw new border
									corner = $(this).parent('div[rel]:first')
											.attr('rel')
									drawBorder.call(self, $(this),
											borders[corner],
						...
						self.elements.tooltip
								.find('.qtip-wrapper [nodeName="arc"]').each(
										function() {
											$(this)
													.attr(
															'fillcolor',
			...
			updateContent : function(content, reposition) {
				...
				// Check if images need to be loaded before position is updated
				...
				function afterLoad() {
					...
					if (reposition !== false) {
						...
						if (self.options.position.type !== 'static')
							self.updatePosition(self.elements.tooltip
											.is(':visible'), true);
			...
			focus : function(event) {
				...
				if (!self.status.focused && curIndex !== newIndex) {
					...
					$('div.qtip[qtip]').not(self.elements.tooltip).each(
							function() {
								if ($(this).qtip('api').status.rendered === true) {
			...
			destroy : function() {
				...
				if (typeof interfaces == 'object' && interfaces.length > 0)
					self.elements.target.data('qtip').current = interfaces.length
							- 1;
				else
	...
	function construct() {
		...
		self.elements.content = self.elements.contentWrapper
				.children('div:first').css(jQueryStyle(self.options.style));
	...
	function positionTip(corner) {
		...
		if (corner.search(/top|bottom/) !== -1) {
			...
			// Adjustments for IE6 - 0.5px border gap bug
			if ($.browser.msie)
				positionAdjust = (parseInt($.browser.version.charAt(0)) === 6)
						? 1
	...
	function createTitle() {
		...
		self.elements.title = $('<div class="'
				+ self.options.style.classes.title + '">').css(jQueryStyle(
				self.options.style.title, true)).css({
		...
		if (self.options.content.title.button !== false
			...
			self.elements.button = $('<a class="'
					...
					.prependTo(self.elements.title).click(function(event) {
								if (!self.status.disabled)
									self.hide(event)
							});
		};
	...
	function assignEvents() {
		...
		if (self.options.position.target === 'mouse'
			...
			showTarget.bind('mousemove.qtip', function(event) {
						...
						// Update the tooltip position only if the tooltip is
	...
	function buildStyle() {
		...
		if (finalStyle.tip.corner === true)
			finalStyle.tip.corner = (self.options.position.corner.tooltip === 'center')
					? false
	...
	$(document).ready(function() {
		...
		// Adjust positions of the tooltips on window resize or scroll if
		// enabled
		var adjustTimer;
		$(window).bind('resize', function(event) {
			...
			adjustTimer = setTimeout(function() {
				...
				for (i = 0; i < $.fn.qtip.interfaces.length; i++) {
					...
					// Update position if resize or scroll adjustments are
					// enabled
					if (api.status.rendered === true
							&& (api.options.position.type !== 'static'
									||  api.options.position.adjust.resize
									&& event.type === 'resize')) {
