...
<!-- <script src="${kmsContextPath }kms/multidoc/resource/js/dialogOption.js"></script>-->
...
<script type="text/javascript">
	...
	function showMore1(obj,num){
		  ...
		  if( $(obj).text()=='收起'){
		      ...
		      $(nextall[num-1]).append("<br id='br1'>");
			  for(jj=num;jj<nextall.length;jj++){
			 		 $(nextall[jj]).show() ;
			 		 }
			  $(obj).text('收起') ;
	...
	function showMore2(obj){
		...
		if(selectMore){
			 $(div).find('br').nextAll().toggle() ;
		 }else{
			 $(div).find('br').nextAll().filter('a').toggle() ;
			 }
		if( $(obj).text()=='收起'){
			  $(obj).text('更多');
			  $(obj).attr("title",'更多')  ;
	...
	function showMore1Button(){
		var size= '<%=((List) request.getAttribute("docTemplateList")).size()%>'; 
		 if( parseInt(size)>param1){
	       $('#templates').prepend("<a  hidefocus='true' id='strHref1'  href='javascript:void(0)' onclick='showMore1(this,"+param1+")' class='more2'  >收起</a>") ;
	       //hideMore1(param1);
	   	 }
	 }
	...
	function backToParent(obj){
		window.open("<c:url value='/kms/multidoc/kms_multidoc_knowledge/kmsMultidocKnowledge.do?method=index&templateId="+obj.id+"' />","_self");
	}
	function backToParentOther(obj){
		window.open("<c:url value='/kms/multidoc/kms_multidoc_knowledge/kmsMultidocKnowledge.do?method=index&filterConfigId=${filterConfigId}&optionId="+obj.id+"' />","_self");
	}
	...
	function gotoFilter(){
		window.open("<c:url value='/kms/multidoc/kms_multidoc_knowledge/kmsMultidocKnowledge.do?method=index&filterConfigId=${filterConfigId}&fdId=${param.fdId}' />","_self");
	}
	...
	function showDocNum(){
		  $(".text_list a").each(function(i) {
			if(this.id) {
				...
				$.getJSON(KMS.contextPath+"kms/multidoc/kms_multidoc_main/jsonp.jsp", {bean: "kmsHomeMultidocService", meth: "calculateDocCount",filterConfigId:"${filterConfigId}", templateId: tid,optionId:this.id,date: jQuery.now()}, function(json){
	...
	function showDataGrid() {
		kmDoc_dataGrid = new DataGrid('kmsDocList','filter', {
			selectable: true, 
			columnModel:[{header: '题目',  dataIndex: 'docSubject'}, 
						{header: '所属分类', dataIndex: 'kmsMultidocTemplate'}, 
						{header: '创建者', dataIndex: 'docCreator'}, 
						{header: '创建时间', dataIndex: 'docCreateTime'}, 
						{header: '浏览次数', dataIndex: 'docReadCount'}, 
						{header: '点评分', dataIndex: 'docScore'}, 
						{header: '部门', dataIndex: 'docDept'}],
			dataSource: {
				url: KMS.contextPath+'kms/multidoc/kms_multidoc_main/jsonp.jsp', 
				data: {bean: "kmsHomeMultidocService", meth: "findDoc",filterConfigId: "${param.filterConfigId}", templateId: "${param.templateId}", propertyId: "${param.propertyId}" , optionId: "${param.optionId}" }
			}
		});
	}
	...
	function showSelectTemplate(doSomething){
	 	 var paramTemplateId="${templateId}";
	     	if( doSomething=='addDoc' && paramTemplateId.length>0 && hasTemplate=="true")  
	         	window.open('<c:url value="/kms/multidoc/kms_multidoc_knowledge/kmsMultidocKnowledge.do?method=add&fdTemplateId=" />'+paramTemplateId,'_blank');
	     	else{
	...
	/**
	 * 是否显示属性修改、权限变更、分类转移按钮
	 */
	function showBtn() {
		var isShow = '${param.templateId}'?"blank":"none";
		$('#editProperty').css("display",isShow);
		$('#changeTemp').css("display",isShow);
		$('#changeRightAll').css("display",isShow);
	}
	...
	function deleteDoc(){
		var baseURI = '<c:url value="/kms/multidoc/kms_multidoc_knowledge/kmsMultidocKnowledge.do?method=deleteall&categoryId=${param.templateId}" />';
	...
	function changeRightAll() {
		var docIds= kmDoc_dataGrid.findSelectedDoc();
		if(docIds == null || docIds == "") {
			return;
		}
		var url="<c:url value="/sys/right/rightDocChange.do"/>";
		url+="?method=docRightEdit&modelName=com.landray.kmss.kms.multidoc.model.KmsMultidocKnowledge&categoryId=${param.templateId}";
		url+="&fdIds="+docIds;
