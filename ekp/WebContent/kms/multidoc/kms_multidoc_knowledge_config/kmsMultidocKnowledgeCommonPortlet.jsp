...
// 从数据库取得数据
	...
	IXMLDataBean bean = (IXMLDataBean) ctx.getBean("kmsMultidocKnowledgeCommonPortlet");
...
<div style="width: 100%;text-align: center;">
	<table style="width: 95%;">
		<tr>
			<td></td>
			<td align="right">
			 	<a href="<c:url value='/kms/multidoc/kms_multidoc_knowledge_config/kmsMultidocKnowledgeConfig.do?method=edit' />" target="_blank">设置</a>
			</td>
		</tr>
		<c:forEach items="${dataList}" var="map" varStatus="vstatus">
			...
			<input id="s_path[${vstatus.index}]" value="${map['s_path']}" type="hidden"/>
				...
				<td  align="right" style="border-bottom: 1px #F0F0F0 dashed ;">
					<img title="<bean:message key="button.add"/>" src="${KMSS_Parameter_StylePath}icons/edit.gif" onclick="Com_OpenWindow('<c:url value="/kms/multidoc/kms_multidoc_knowledge/kmsMultidocKnowledge.do" />?method=add&fdTemplateId=${map['id']}');" 
