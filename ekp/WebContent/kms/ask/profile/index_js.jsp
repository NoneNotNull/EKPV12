		...
		$
				.ajax( {
					...
					success : function(data) {
						$('#list').html(
								KmsTmpl($('#portlet_doc_kmservice_tmpl').html())
										.render( {
											"data" : data
										}));
					}
...
{$
<div class="km_list2">
<table width="100%" cellspacing="0" cellpadding="0" border="0" class="t_g" id="tbObj">
	<thead class="t_g_a">
		<tr class="tal">
			<th width="5%">NO.</th>
			<th width="10%">悬赏币</th>
...
{$
	<tr 
$}
		if(i%2!=0){{$ class="t_g_c" $}}{$>
		<td>{%i+1%}</td>
		<td><span class="score">{%itemList[i].fdScore%}</span></td>
		<td><a title="{%itemList[i].docSubject%}" href="{%itemList[i].fdUrl%}" target="_blank">{%resetStrLength(itemList[i].docSubject,30)%}</a></td>
		<td><span class="answer_num">{%itemList[i].fdReplyCount%} 回答</span></td>
		<td><span class="{%itemList[i].fdStatus > 0 ? 'state' : 'state2' %}"></span></td>
