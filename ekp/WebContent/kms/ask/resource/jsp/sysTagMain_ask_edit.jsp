...
<script language="JavaScript">
	function submitTagApp(){
		var queryConditionName = '${param.fdQueryCondition}';
...
<c:set var="sysTagMainForm" value="${requestScope[param.formName].sysTagMainForm}" />
	...
	<html:hidden property="sysTagMainForm.fdQueryCondition"/> 
		...
		<html:text property="sysTagMainForm.fdTagNames" styleClass="i_b"  onfocus="showTagApplication(true);" style="width:55%;"/>
		<span>
		 <a href="#" onclick="Dialog_Tree(true,null,null,' ','sysTagCategorApplicationTreeService&fdCategoryId=!{value}','<bean:message key="sysTagTag.tree" bundle="sys-tag"/>',false,afterSelectValue,null,false,true,null)">
		 <span><bean:message key="dialog.selectOther"/>
 		  </span></a></span>
		<div id = "id_application_div"  style="padding-left:5px; margin-left:5px;width:55%;display:none;background:#FFFFCC">
				<div><a id="a_close" href="#" onclick="showTagApplication(false);" style="float: right;"><font size="5px">×</font></a></div>
				<div style="width:97%;height:20%"><b><bean:message bundle="sys-tag" key="sysTagMain.message.0"/></div>
				<div id = "hot_id" style="width:100%;margin-top:8px;"><bean:message bundle="sys-tag" key="sysTagMain.message.1"/></div>
				<div id = "used_id" style="width:100%;margin-top:8px;margin-bottom:10px;"><bean:message bundle="sys-tag" key="sysTagMain.message.2"/></div>			
		</div>
