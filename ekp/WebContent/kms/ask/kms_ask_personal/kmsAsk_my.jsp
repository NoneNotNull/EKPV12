...
<%@ include file="/sys/ui/jsp/common.jsp"%>
		...
		<list:criteria id="kmsAskCriteria" expand="true" channel="ask">		
			...
			<list:cri-criterion title="${lfn:message('kms-ask:kmsAskTopic.fdmyknow') }" key="myknow">
				<list:box-select>
					<list:item-select cfg-defaultValue="myask" cfg-required="true">
						<ui:source type="Static">
							[{text:'${ lfn:message('kms-ask:kmsAsk.zone.ask.my') }', value:'myask'},
							{text:'${ lfn:message('kms-ask:kmsAsk.zone.answer.my') }',value:'myanswer'}]
						</ui:source>
					</list:item-select>
				</list:box-select>
			</list:cri-criterion>
		...
		<list:listview id="listview1" channel="ask">
						<ui:source type="AjaxJson">
							{url:'/kms/ask/kms_ask_index/kmsAskTopicIndex.do?method=index'}
						</ui:source>
						...
						<list:colTable layout="sys.ui.listview.columntable" name="columntable"
							...
							<list:col-html title="${ lfn:message('kms-ask:kmsAskTopic.docSubject')}" headerStyle="width:40%" style="text-align:left;padding:0 8px" >
								{$
