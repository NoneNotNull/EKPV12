...
<div id="optBarDiv">
	<c:if test="${kmsIntegralPersonFilterForm.method_GET=='edit'}">
		<input type=button value="<bean:message key="button.update"/>"
			onclick="if(submited())Com_Submit(document.kmsIntegralPersonFilterForm, 'update');">
	</c:if>
	<c:if test="${kmsIntegralPersonFilterForm.method_GET=='add'}">
		<input type=button value="<bean:message key="button.save"/>"
			onclick="if(submited())Com_Submit(document.kmsIntegralPersonFilterForm, 'save');">
		<input type=button value="<bean:message key="button.saveadd"/>"
			onclick="if(submited())Com_Submit(document.kmsIntegralPersonFilterForm, 'saveadd');">
	</c:if>
...
<table class="tb_normal" width=60%>
	...
	<tr>
		<td class="td_normal_title">
			<bean:message bundle="kms-integral" key="kmsIntegralCommon.personal"/>
		</td><td>
			<input type="hidden" name="fdDeptIds" value="${kmsIntegralPersonFilterForm.fdDeptIds }">
