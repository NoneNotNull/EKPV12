...
<c:if test="${sysWfBusinessForm.sysWfBusinessForm!=null}">
	<%@ include file="/kms/common/pda/core/lbpm/lbpm_view_script.jsp"%>
	<c:if test="${requestScope.sysWfProcess_useActionView != 'true'}">
		...
		<table class="tb_normal" width="100%">
			<c:if test="${(sysWfBusinessForm.docStatus>='20' && sysWfBusinessForm.docStatus<'30') || sysWfBusinessForm.docStatus == '11'}">
				...
				<%
					ExtendForm lbpmForm = (ExtendForm)pageContext.getAttribute("sysWfBusinessForm");
					Class claz = lbpmForm.getModelClass();
					String url = ModelUtil.getModelUrl(lbpmForm, claz.getName());
					session.setAttribute("S_DocLink", url);
				%>
				<!-- 处理事务 -->
				...
				<tr style="cursor: pointer;" onclick="Com_OpenWindow('<c:url value="/kms/common/pda/core/lbpm/lbpmProcess_flowchart.jsp?processId=${sysWfBusinessForm.fdId}"/>','_self');">
					<td class="td_title">
