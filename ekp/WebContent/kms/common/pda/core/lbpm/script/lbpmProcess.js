Com_IncludeFile("data.js|formula.js");
...
lbpm.globals.parseProcessorObj=function(){
	...
	//当前节点的详版XML
	if(curNodeXMLObj.nodes && curNodeXMLObj.nodes.length>0){
		for(var i=0,size=curNodeXMLObj.nodes.length;i<size;i++){
			for(o in curNodeXMLObj.nodes[i]){
				if(lbpm.nodes[curNodeXMLObj.nodes[i].id]){
					lbpm.nodes[curNodeXMLObj.nodes[i].id][o]=curNodeXMLObj.nodes[i][o];
				}				
			}
		}
	}
	var drafterInfoObj = lbpm.globals.getDrafterInfoObj(curNodeXMLObj);
	var authorityInfoObj = lbpm.globals.getAuthorityInfoObj(curNodeXMLObj);
	lbpm.drafterInfoObj=drafterInfoObj; //当前用户以起草人身份所拥有的信息
	lbpm.authorityInfoObj=authorityInfoObj;//当前用户以特权人身份所拥有的信息
...
levelCalc=function(startNodeId,level,allNodes){
	if(!allNodes){
		allNodes=[];
	}
	if(!level){
	...
	for(var i=0;i<nextNodes.length;i++){
		...
		levelCalc(nextNodes[i].id,level,allNodes);
	}
}
...
/*
 * 解析简版XML成lbpm对象nnn
 * @param isInitPage 是否是初始化页面
 */
lbpm.globals.parseXMLObj=function(isInitPage)
{
	//解析流程的XML成对象
	...
	// 节点排序相关
	var allNodes = $.grep(processData.nodes, function() {return true;});
	var allLines = processData.lines;
	...
	//取当前节点的信息（即使当前用户不是处理人也有此信息）
	/*var value=document.getElementById("sysWfBusinessForm.fdNodeInfoXML").value;
	if(value!=""){
		var nodeInfoObj = WorkFlow_LoadXMLData(value);
		lbpm.draftorName=nodeInfoObj.draftorName;
		lbpm.draftorId=nodeInfoObj.draftorId;
		lbpm.historyHandlers=nodeInfoObj.historyHandlers;
		//当前处理人，如果是串行节点有三个人审批，这个属性只取其中一个人（正在审批的），取姓名，不取ID
		lbpm.currentHandlers=nodeInfoObj.currentHandlers;
	}	*/
...
lbpm.globals.getWfBusinessFormModelId=function() {
	...
	if (modelId == '') {
		...
		if (fdModelId.length > 0) {
			modelId = fdModelId[0].value;
		}
	}
	return modelId;
};
...
//取得下一节点的对象数组
lbpm.globals.getNextNodeObjs=function (nodeId, objArray){
	var nodeObj=lbpm.nodes[nodeId];
...
lbpm.globals.parseTasksInfo=function(curNodeXMLObj,taskFrom,identity){
	...
	for(var i=0,size=_tasksArr.length;i<size;i++){
		if(_tasksArr[i].operations){
			//过滤操作
			var arr=$.grep(_tasksArr[i].operations,function(n,i){
				 return n.operationHandlerType==identity;
			});
			if(arr.length==0)
				_tasksArr[i]=null;
			else
				_tasksArr[i].operations=arr;			
		}else{
			_tasksArr[i]=null;
		}

	};
	var rtnArr=$.grep(_tasksArr,function(n,i){
		 return n!=null;
	});
	return rtnArr;
}
...
lbpm.globals.outer_GetCurrentNodeInfo=function(){	
	var currentNodeObj = lbpm.globals.getCurrentNodeObj();
	return currentNodeObj.XMLNODENAME;
};
...
/*
 是否有审批人权限（转办、会审、会签）
*/
lbpm.globals.outer_HasProcessorRole=function(){
	...
	if(processorInfoObj != null && processorInfoObj.length > 0){
		for(var i = 0; i < processorInfoObj.length; i++){
			var hasRtn=false
			var oprNames=processorInfoObj[i].operations;
			for(var j=0,size=oprNames.length;j<size;j++){
				if(oprNames.id==HANDLER_OPERATION_TYPE_RTNCOMMUNICATE) hasRtn=true;
			}
			if(hasRtn) return false; else return true;
		}
	}
	return false;	
};
...
(function(lbpm){
	...
	var jsPath=dir+"syslbpmprocess_event.js|"+dir+"syslbpmprocess_submit.js|"+dir+"syslbpmprocess_nodes_filter.js|"+dir+"concurrency.js";
