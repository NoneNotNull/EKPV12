...
artDialog.fn = artDialog.prototype = {
	...
	_init: function (config) {
		var that = this, DOM,
			...
			/* 修改skin path路径 */
			: {backgroundImage: 'url(\'' + KMS.themePath + '/artDialog/icons/' + icon + '.png\')'});
	...
	_pngFix: function () {
		var i = 0, elem, png, pngPath, runtimeStyle,
			path = artDialog.defaults.path + '/artDialog/',
...
// 获取artDialog路径
_path = window['_artDialog_path'] || KMS.themePath;
...
// 无阻塞载入CSS (如"artDialog.js?=aero")
_skin = _thisScript.src.split('skin=')[1];
if (_skin) {
	var link = document.createElement('link');
	link.rel = 'stylesheet';
	link.href = KMS.themePath + "/artDialog/" + _skin + '.css';
