...
{$ 	
	...
	<div id="logo" $} if(data.bgImg){ {$ style="background-image:url('{%data.bgImg%}')" $} } {$><div class="box" id="rightImg" $} if(data.rightImg){ {$ style="background-image:url('{%data.rightImg%}')" $} } {$>
		<div class="logo_box c">
			<a href="$}if(data.fdLogoUrl){{$ {%data.fdLogoUrl%} $} }else{ {$ javascript:void(0) $} } {$" class="logo" $} if(data.fdLogoUrl){ {$ target="_blank" $} } {$ title="" style="FILTER:progid:DXImageTransform.Microsoft.AlphaImageLoader(sizingMethod=image);width:184px;height:46px;" title=""><img src="$} if(data.logoImg){{$ {%data.logoImg%} $}}else{{$ ${kmsThemePath}/img/logo.gif $}} {$" id="logoImg" style="width:184px;height:46px;" ></a>
			<ul class="search_box" style="display: $} if(data.fdSearchDisplay ||typeof(data.fdSearchDisplay)=='undefined'){{$ block $} } else { {$ none $} }{$">
				<li class="search"><input type="text" class="input_search" id="searchTextInput" value="请输入关键字" onfocus="if(value=='请输入关键字'){value=''}" onblur="if(value==''){value='请输入关键字'}"><a href="#" id="btnSearch" class="btn_search" title="搜索">搜索</a><a href="#" id="btnSearchAdvance" class="btn_gaoji" title="高级">高级</a>	
...
$}
				for(i=0;i<data.navList.length;i++){
	     			{$<li {%data.fdId==data.navList[i].fdId?'class=on':''%}><a href="{%data.navList[i].fdPath%}" target="{%data.navList[i].fdTarget%}" >{%data.navList[i].fdName%}</a>{%i==data.navList.length-1?'':'|'%}</li>$}
				}
{$ 		
		</ul>
...
{$
	<div class="title1"><h2 class="h2_2">{% parameters.kms.title %}</h2></div>
	<div>
		<ul class="l_c">
$}
...
{$ 
	<div class="title1"><h2 class="h2_1">{% parameters.kms.title %}
$}
	if(data.more){
{$
