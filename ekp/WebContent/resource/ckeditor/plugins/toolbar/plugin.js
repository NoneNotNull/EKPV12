...
(function() {
	...
	CKEDITOR.plugins.add('toolbar', {
		...
		init : function(editor) {
			...
			var itemKeystroke = function(item, keystroke) {
				...
				toolbarGroupCycling = toolbarGroupCycling === undefined
						|| toolbarGroupCycling;
				...
				switch (keystroke) {
					...
					case CKEDITOR.SHIFT + 9 : // SHIFT + TAB
						...
						while (!toolbar || !toolbar.items.length) {
							toolbar = keystroke == 9
									? ((toolbar
											? toolbar.next
											: item.toolbar.next) || editor.toolbox.toolbars[0])
									: ((toolbar
											? toolbar.previous
											: item.toolbar.previous) || editor.toolbox.toolbars[editor.toolbox.toolbars.length
											- 1]);

							// Look for the first item that accepts focus.
							if (toolbar.items.length) {
								item = toolbar.items[endFlag
										? (toolbar.items.length - 1)
										: 0];
					...
					case 38 : // UP-ARROW
						...
						do {
							...
							if (!next && toolbarGroupCycling)
								next = item.toolbar.items[item.toolbar.items.length
										- 1];
						} while (next && !next.focus)
			...
			editor.on('uiSpace', function(event) {
				...
				var output = [
						...
						labelId, '" onmousedown="return false;">'];
				...
				for (var r = 0; r < toolbar.length; r++) {
					...
					for (var i = 0; i < items.length; i++) {
						var item = items[i], canGroup;

						if (item) {
							if (item.type == CKEDITOR.UI_SEPARATOR) {
								// Do not add the separator immediately. Just
								// save
								// it be included if we already have something
								// in
								// the toolbar and if a new item is to be added
								// (later).
							...
							if (!toolbarObj) {
								...
								toolbarName = row.name
										&& (editor.lang.toolbar.toolbarGroups[row.name] || row.name);

								// Output the toolbar opener.
								output.push('<span id="', toolbarId,
										'" class="cke_toolbar"', (toolbarName
												? ' aria-labelledby="'
														+ toolbarId + '_label"'
												: ''), ' role="toolbar">');

								// If a toolbar name is available, send the
								// voice label.
								toolbarName
										&& output
												.push(
								...
								// Add the toolbar to the
								// "editor.toolbox.toolbars"
								// array.
							...
							function addItem(item) {
								...
								if (index > 0) {
									itemObj.previous = toolbarObj.items[index
											- 1];
								...
								// Fix for #3052:
								// Prevent JAWS from focusing the toolbar after
								// document load.
				...
				if (editor.config.toolbarCanCollapse
					...
					editor.addCommand('toolbarCollapse', {
						readOnly : 1,
						exec : function(editor) {
							var collapser = editor.ui
									.space('toolbar_collapser'), toolbox = collapser
									.getPrevious(), contents = editor.ui
									.space('contents'), toolboxContainer = toolbox
									.getParent(), contentHeight = parseInt(
									contents.$.style.height, 10), previousHeight = toolboxContainer.$.offsetHeight, minClass = 'cke_toolbox_collapser_min', collapsed = collapser
									.hasClass(minClass);

							if (!collapsed) {
								toolbox.hide();
								collapser.addClass(minClass);
								collapser.setAttribute('title',
							...
							collapser.getFirst().setText(collapsed ? '\u25B2' : // BLACK
									// UP-POINTING
									// TRIANGLE
									'\u25C0'); // BLACK LEFT-POINTING TRIANGLE

							var dy = toolboxContainer.$.offsetHeight
									- previousHeight;
							contents.setStyle('height', (contentHeight - dy)
											+ 'px');
					...
					editor.setKeystroke(CKEDITOR.ALT
					...
					output.push('" onclick="CKEDITOR.tools.callFunction('
									+ collapserFn + ')">',
							'<span class="cke_arrow">&#9650;</span>', // BLACK
							// UP-POINTING
							// TRIANGLE
							'</a>');
				}
	...
	function getToolbarConfig(editor) {
		...
		function buildToolbarConfig() {
			...
			var toolbar = CKEDITOR.tools.clone(editor.config.toolbarGroups)
		...
		function getItemDefinedGroups() {
			...
			for (group in groups) {
				groups[group] = groups[group].sort(function(a, b) {
							return a.order == b.order ? 0 : b.order < 0
									? -1
									: a.order < 0 ? 1 : a.order < b.order
											? -1
											: 1;
						});
			}
	...
	/**
	 * Add toolbar group. See {@link CKEDITOR.config#toolbarGroups} for more
	 * details.
	 * 
	 * **Note:** This method won't modify toolbar groups set explicitly by
	 * {@link CKEDITOR.config#toolbarGroups}. It will extend only default
	 * setting.
	 * 
	 * @param {String}
	 *            name Group name.
	 * @param {Number/String}
	 *            previous Name of group after which this one should be added or
	 *            `0` if this group should be the first one.
	 * @param {String}
	 *            [subgroupOf] Name of parent group.
	 * @member CKEDITOR.ui
	 */
	CKEDITOR.ui.prototype.addToolbarGroup = function(name, previous, subgroupOf) {
		// The toolbarGroups from the privates is the one we gonna use for
		// automatic toolbar creation.
		var toolbarGroups = getPrivateToolbarGroups(this.editor), atStart = previous === 0, newGroup = {
			name : name
		};
		...
		if (subgroupOf) {
			...
			if (subgroupOf) {
				...
				if (previous) {
					...
					previous = CKEDITOR.tools.indexOf(subgroupOf.groups,
							previous);
					if (previous >= 0) {
...
/**
 * The "UI space" to which rendering the toolbar. For the default editor
 * implementation, the recommended options are `'top'` and `'bottom'`.
 * 
 ...
 * toolbars (strips), each one being also an array, containing a list of UI
 * items.
 * 
 * If set to `null`, generate toolbar automatically using all available buttons
 * and {@link #toolbarGroups} as a toolbar groups layout. // Defines a toolbar
 * with only one strip containing the "Source" button, a // separator and the
 * "Bold" and "Italic" buttons. config.toolbar = [ [ 'Source', '-', 'Bold',
 * 'Italic' ] ]; // Similar to example the above, defines a "Basic" toolbar with
 * only one strip containing three buttons. // Note that this setting is
 * composed by "toolbar_" added by the toolbar name, which in this case is
 * called "Basic". // This second part of the setting name can be anything. You
 * must use this name in the CKEDITOR.config.toolbar setting, // so you instruct
 * the editor which toolbar_(name) setting to use. config.toolbar_Basic = [ [
 * 'Source', '-', 'Bold', 'Italic' ] ]; // Load toolbar_Name where Name = Basic.
 ...
 * this setting is used to group all defined buttons (see
 * {@link CKEDITOR.ui#addButton}). Buttons are associated with toolbar groups
 * by `toolbar` property in their definition objects.
 * 
 * New groups may be dynamically added during the editor and plugins
 * initialization by {@link CKEDITOR.ui#addToolbarGroup}. Although only if
 * default setting was used. // Default setting. config.toolbarGroups = [ {
 * name: 'document', groups: [ 'mode', 'document', 'doctools' ] }, { name:
 * 'clipboard', groups: [ 'clipboard', 'undo' ] }, { name: 'editing', groups: [
 * 'find', 'selection', 'spellchecker' ] }, { name: 'forms' }, '/', { name:
 * 'basicstyles', groups: [ 'basicstyles', 'cleanup' ] }, { name: 'paragraph',
 * groups: [ 'list', 'indent', 'blocks', 'align' ] }, { name: 'links' }, { name:
 * 'insert' }, '/', { name: 'styles' }, { name: 'colors' }, { name: 'tools' }, {
 ...
 * 
 * config.toolbarCanCollapse = true; config.toolbarStartupExpanded = false;
 * 
 ...
 * {@link CKEDITOR.config#toolbar} if it is set or automatically based on
 * {@link CKEDITOR.config#toolbarGroups}.
 * 
 * @readonly
