...
CKEDITOR.dialog.add("paste", function(c) {
	function h(a) {
		var b = new CKEDITOR.dom.document(a.document), f = b.getBody(), d = b
		...
		function ajax_upload(d) {
			$.ajax({
						url : config.filebrowserImageUploadUrl,
						data : d,
						contentType : false,
						processData : false,
						type : "POST",
						success : function(html) {
							...
							img.setAttribute('src', html);
							f.append(img);
						}
					});
		}
		function imageUpload__(items, loop) {
			for (var i = 0; i < items.length; ++i) {
				var item = items[i];
				if (items[i].type.indexOf('image') >= 0) {
					var d = new FormData;
					d.append("parst", true);
					d.append("NewFile", item.getAsFile());
					ajax_upload(d);
					if (!loop)
						break;
				}
	...
	return {
		...
		contents : [{
			...
			elements : [{
				...
				html : "",
				focus : function() {
					var a = this.getInputElement(), b = a.getFrameDocument()
							.getBody();
					!b || b.isReadOnly()
				...
				setup : function() {
					var a = this.getDialog(), b = '<html dir="'
							...
							+ ",this);<\/script></body></html>", f = CKEDITOR.env.air
							? "javascript:void(0)"
							: CKEDITOR.env.ie ? "javascript:void((function(){"
					...
					CKEDITOR.env.ie
							&& (a.setStyle("display", "block"), a.setStyle(
									"height", d.$.offsetHeight + 2 + "px"))
				},
				commit : function() {
					var a = this.getDialog().getParentEditor(), b = this
							.getInputElement().getFrameDocument().getBody(), c = b
							.getBogus(), d;
					c && c.remove();
					d = b.getHtml();
					setTimeout(function() {
