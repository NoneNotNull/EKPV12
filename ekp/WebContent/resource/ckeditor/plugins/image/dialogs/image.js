...
function SetUrl(url, width, height, alt) {
	var dialog = CKEDITOR.dialog.getCurrent();
	dialog.setValueOf('info', 'txtUrl', url ? url : '');
	resetSize();
}
...
function resetSize() {
	var dialog = CKEDITOR.dialog.getCurrent();
	var ___orign = dialog.originalElement;
	if (___orign.getCustomData("isReady") == 'false') {
		setTimeout(resetSize, 50);
		return;
	}
	var ___height = ___orign.$.height, ___width = ___orign.$.width;
	dialog.setValueOf('info', 'txtHeight', ___height ? ___height : '');
	dialog.setValueOf('info', 'txtWidth', ___width ? ___width : '');
}
...
function CheckUpload(evt) {
	...
	var oUploadAllowedExtRegex = new RegExp(
			CKconfig.ImageUploadAllowedExtensions, 'i');
	var oUploadDeniedExtRegex = new RegExp(
			CKconfig.ImageUploadDeniedExtensions, 'i');

	if ((CKconfig.ImageUploadAllowedExtensions.length > 0 && !oUploadAllowedExtRegex
			.test(sFile))
			|| (CKconfig.ImageUploadDeniedExtensions.length > 0 && oUploadDeniedExtRegex
					.test(sFile))) {
...
// 大小超过限制--admin.do中配置
function OnUploadMaxSizeLimit(size, maxSize) {
	CKEDITOR.dialog.getCurrent().reset();
	alert('图片附件超过系统设定大小限制'+ maxSize +'，请重新上传');
}
