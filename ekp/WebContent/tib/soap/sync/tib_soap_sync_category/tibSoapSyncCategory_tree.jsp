...
{
	LKSTree = new TreeView("LKSTree", "<bean:message key="table.tibSoapSyncCategory" bundle="tib-soap-sync"/>", document.getElementById("treeDiv"));
	LKSTree.isShowCheckBox=true;
	LKSTree.isMultSel=true;
	LKSTree.isAutoSelectChildren = false;
	var n1, n2;
	n1 = LKSTree.treeRoot;
	n2 = n1.AppendBeanData("tibSoapSyncCategoryTreeService&parentId=!{value}");
	//TODO 数据显示不出来
	LKSTree.Show();
	  var  iframeObj=document.getElementById("iframeSearch");//搜索结果页面
 	var url="<c:url value='/tib/soap/sync/tib_soap_sync_category/tibSoapSyncCategory.do?method=add'/>"; 
	iframeObj.src=url;
}
...
//新建
	...
	if(checkedNode.length>0){
	   ...
	   {
	   alert("<bean:message key="error.messageone" bundle="tib-soap-sync"/>");
	   return false;
	   }
...
function editCategory(){
	var url = "<c:url value="/tib/soap/sync/tib_soap_sync_category/tibSoapSyncCategory.do" />?method=edit&mainModelName=${param.mainModelName}";
	var checkedNode = LKSTree.GetCheckedNode();
	if(List_CheckSelect()){
	if(checkedNode.length>1)
	   {
...
function deleteCategories(){
	...
	for(var i=selList.length-1;i>=0;i--){
		...
		document.tibSoapSyncCategoryForm.appendChild(input);	
		
	}
	Com_Submit(document.tibSoapSyncCategoryForm, 'deleteall');	
}
//选择
