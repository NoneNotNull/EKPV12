...
{
	LKSTree = new TreeView("LKSTree", "<bean:message key="tibJdbcDataSetCategory.categoryConfig"  bundle="tib-jdbc"/>", document.getElementById("treeDiv"));
	LKSTree.isShowCheckBox=true;
	LKSTree.isMultSel=true;
	LKSTree.isAutoSelectChildren = false;
	var n1, n2;
	n1 = LKSTree.treeRoot;
	n2 = n1.AppendBeanData("tibJdbcDataSetCategoryTreeService&parentId=!{value}");//选取树数据的bean
	//TODO 数据显示不出来
	LKSTree.Show();
	var  iframeObj=document.getElementById("iframeSearch");//搜索结果页面
 	var url="<c:url value='/tib/jdbc/tib_jdbc_data_set_category/tibJdbcDataSetCategory.do?method=add'/>"; 
	iframeObj.src=url;
}
...
//新建
	...
	if(checkedNode.length>0){
	   ...
	   {
		   alert("<bean:message key="error.messageone" bundle="tib-jdbc"/>");
		   return false;
	   }
		if(checkedNode[0].nodeType=="CATEGORY_SON"){
			alert("<bean:message key="error.illegalCreateCategory" bundle="tib-jdbc"/>");
			return false;
		}else{
...
function editCategory(){
	var url = "<c:url value='/tib/jdbc/tib_jdbc_data_set_category/tibJdbcDataSetCategory.do' />?method=edit&mainModelName=${param.mainModelName}";
	var checkedNode = LKSTree.GetCheckedNode();
	if(List_CheckSelect()){
	if(checkedNode.length>1)
	   {
...
function viewCategory(id) {
	...
	if(node!=null && node.value!=null) {
		var url = "<c:url value='/tib/jdbc/tib_jdbc_data_set_category/tibJdbcDataSetCategory.do' />?method=view&fdId="+node.value;
		Com_OpenWindow(url);
	}
}
// 删除
function deleteCategories(){
	...
	for(var i=selList.length-1;i>=0;i--){
		...
		document.tibJdbcDataSetCategoryForm.appendChild(input);	
		
	}
	Com_Submit(document.tibJdbcDataSetCategoryForm, 'deleteall');	
}
//选择
