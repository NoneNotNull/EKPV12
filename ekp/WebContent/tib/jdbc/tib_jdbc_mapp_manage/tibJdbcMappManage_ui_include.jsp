	...
	<template:replace name="path">
		<ui:combin ref="menu.path.tib.simplecategory">
			<ui:varParams 
				...
				cateTitle="${ lfn:message('tib-jdbc:table.tibJdbcMappManage') }" 
				href="javascript:parent.tibLoadList('/tib/jdbc/tib_jdbc_mapp_manage/tibJdbcMappManage_ui_include.jsp?categoryId=!{value}');" 
	...
	<template:replace name="content">
		...
		<list:criteria>
			...
			<list:cri-ref ref="criterion.sys.simpleCategory" key="docCategory" multi="false" title="分类导航" expand="true">
			  <list:varParams modelName="com.landray.kmss.tib.jdbc.model.TibJdbcMappCategory"/>
			</list:cri-ref>
			<list:cri-auto modelName="com.landray.kmss.tib.jdbc.model.TibJdbcMappManage" property="fdIsEnabled"/>
		</list:criteria>
					...
					<td align="right">
						<ui:toolbar>
							...
							<kmss:auth requestURL="/tib/jdbc/tib_jdbc_mapp_manage/tibJdbcMappManage.do?method=deleteall">
								<ui:button text="${lfn:message('button.delete')}" order="4" onclick="delDoc('${LUI_ContextPath}/tib/jdbc/tib_jdbc_mapp_manage/tibJdbcMappManage.do?method=deleteall')"></ui:button>
							</kmss:auth>
						</ui:toolbar>						
					</td>
				</tr>
		...
		<list:listview id="listview">
			<ui:source type="AjaxJson">
				{url:'/tib/jdbc/tib_jdbc_mapp_manage/tibJdbcMappManageIndex.do?method=list&categoryId=${param.categoryId }'}
			</ui:source>
			<%--列表形式--%>
			<list:colTable isDefault="false" layout="sys.ui.listview.columntable" 
				rowHref="/tib/jdbc/tib_jdbc_mapp_manage/tibJdbcMappManage.do?method=view&fdId=!{fdId}" name="columntable">
		...
		seajs.use(['lui/jquery', 'lui/util/str', 'lui/dialog','lui/topic'], function($, strutil, dialog , topic) {
			...
			window.addDocByCate = function(){
				parent.addDoc('${LUI_ContextPath}/tib/jdbc/tib_jdbc_mapp_manage/tibJdbcMappManage.do?method=add&fdtemplatId='+ cateId);
			};
			//根据筛选器分类异步校验权限
