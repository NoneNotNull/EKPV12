...
{
	LKSTree = new TreeView("LKSTree", "<bean:message key="tree.node.categoryConfig" bundle="tib-sys-sap-connector"/>", document.getElementById("treeDiv"));
	LKSTree.isShowCheckBox=true;
	LKSTree.isMultSel=true;
	LKSTree.isAutoSelectChildren = false;
	var n1, n2;
	n1 = LKSTree.treeRoot;
	n2 = n1.AppendBeanData("tibSysSapRfcCategoryTreeService&parentId=!{value}");
	//TODO 数据显示不出来
	LKSTree.Show();
	  var  iframeObj=document.getElementById("iframeSearch");//搜索结果页面
 	var url="<c:url value='/tib/sys/sap/connector/tib_sys_sap_rfc_category/tibSysSapRfcCategory.do?method=add'/>"; 
	iframeObj.src=url;
}
...
//新建
	...
	if(checkedNode.length>0){
	   ...
	   {
	   alert("<bean:message key="error.messageone" bundle="tib-sys-sap-connector"/>");
	   return false;
	   }
...
function editCategory(){
	var url = "<c:url value="/tib/sys/sap/connector/tib_sys_sap_rfc_category/tibSysSapRfcCategory.do" />?method=edit&mainModelName=${param.mainModelName}";
	var checkedNode = LKSTree.GetCheckedNode();
	if(List_CheckSelect()){
	if(checkedNode.length>1)
	   {
...
function viewCategory(id) {
	...
	if(node!=null && node.value!=null) {
		var url = "<c:url value="/tib/sys/sap/connector/tib_sys_sap_rfc_category/tibSysSapRfcCategory.do" />?method=view&fdId="+node.value;
		Com_OpenWindow(url);
	}
}
// 删除
function deleteCategories(){
	...
	for(var i=selList.length-1;i>=0;i--){
		...
		input.name="List_Selected_Node";	//IE表单在DOM结构被修改后，checkbox和radio取值问题  
		input.value = selList[i].value;
		document.tibSysSapRfcCategoryForm.appendChild(input);	
		
	}
	Com_Submit(document.tibSysSapRfcCategoryForm, 'deleteall');	
}
//选择
