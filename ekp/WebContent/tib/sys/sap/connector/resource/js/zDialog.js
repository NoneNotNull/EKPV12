  ...
  <table id="_DialogTable_' + this.ID + '" width="' + (this.Width + 26) + '" cellspacing="0" cellpadding="0" border="0" onselectstart="return false;" style="-moz-user-select: none; font-size:12px; line-height:1.4;">\
    ...
    <tr valign="top">\
      ...
      <td align="center"><table width="100%" cellspacing="0" cellpadding="0" border="0" bgcolor="#ffffff">\
          ...
          <tr>\
            <td valign="top" align="center"><div id="_Container_' + this.ID + '" style="position: relative; width: ' + (this.Width-120) + 'px; height: ' + (this.Height-120) + 'px;">\
        ...
        return "";
          ...
          <tr id="_ButtonRow_' + this.ID + '" style="' + (this.ShowButtonRow ? "" : "display:none") + '">\
            <td height="36"><div id="_DialogButtons_' + this.ID + '" style="border-top: 1px solid #DADEE5; padding: 8px 20px; text-align: center; background-color:#f6f6f6;">\
                <input type="button" class="buttonStyle" value="&nbsp是 &nbsp" id="_ButtonOK_' + this.ID + '"/>\
...
//在最后追加按钮
Dialog.prototype.addButtonAfter = function (id, txt, func){
	  topWin.$id("_ButtonRow_" + this.ID).style.display = "";
	    this.ShowButtonRow = true;
	    var button = topDoc.createElement("input");
	    button.id = "_Button_" + this.ID + "_" + id;
	    ...
	    button.value = txt;
	    button.onclick = func;
	    var countNum=topWin.$id("_DialogButtons_" + this.ID).getElementsByTagName("INPUT").length;
	    var input0 = topWin.$id("_DialogButtons_" + this.ID).getElementsByTagName("INPUT")[countNum-1];
	    input0.parentNode.appendChild(button, input0);
	    return button;
}

//在指定位置追加按钮
Dialog.prototype.addButtonByCondition = function (id, txt, func,index){
	    topWin.$id("_ButtonRow_" + this.ID).style.display = "";
	    this.ShowButtonRow = true;
	    var button = topDoc.createElement("input");
	    button.id = "_Button_" + this.ID + "_" + id;
	    ...
	    if(index<=countNum && index>0){
	    	input0= topWin.$id("_DialogButtons_" + this.ID).getElementsByTagName("INPUT")[index-1];
	    }else{
	    	input0 = topWin.$id("_DialogButtons_" + this.ID).getElementsByTagName("INPUT")[countNum-1];
	    }
	    input0.parentNode.insertBefore(button, input0);
	    return button;
