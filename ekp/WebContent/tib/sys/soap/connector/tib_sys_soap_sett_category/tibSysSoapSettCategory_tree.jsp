...
{
	LKSTree = new TreeView("LKSTree", "<bean:message key="table.tibSysSoapSettCategory" bundle="tib-sys-soap-connector"/>", document.getElementById("treeDiv"));
	LKSTree.isShowCheckBox=true;
	LKSTree.isMultSel=true;
	LKSTree.isAutoSelectChildren = false;
	var n1, n2;
	n1 = LKSTree.treeRoot;
	n2 = n1.AppendBeanData("tibSysSoapSettCategoryTreeService&parentId=!{value}");//选取树数据的bean
	//TODO 数据显示不出来
	LKSTree.Show();
	var  iframeObj=document.getElementById("iframeSearch");//搜索结果页面
 	var url="<c:url value='/tib/sys/soap/connector/tib_sys_soap_sett_category/tibSysSoapSettCategory.do?method=add'/>"; 
	iframeObj.src=url;
}
...
//新建
	...
	if(checkedNode.length>0){
	   ...
	   {
	   alert("<bean:message key="error.messageone" bundle="tib-sys-soap-connector"/>");
	   return false;
	   }
...
function editCategory(){
	var url = "<c:url value="/tib/sys/soap/connector/tib_sys_soap_sett_category/tibSysSoapSettCategory.do" />?method=edit&mainModelName=${param.mainModelName}";
	var checkedNode = LKSTree.GetCheckedNode();
	if(List_CheckSelect()){
	if(checkedNode.length>1)
	   {
...
function viewCategory(id) {
	...
	if(node!=null && node.value!=null) {
		var url = "<c:url value="/tib/sys/soap/connector/tib_sys_soap_sett_category/tibSysSoapSettCategory.do" />?method=view&fdId="+node.value;
		Com_OpenWindow(url);
	}
}
// 删除
function deleteCategories(){
	...
	for(var i=selList.length-1;i>=0;i--){
		...
		document.tibSysSoapSettCategoryForm.appendChild(input);	
		
	}
	Com_Submit(document.tibSysSoapSettCategoryForm, 'deleteall');	
}
//选择
