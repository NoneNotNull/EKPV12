<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<%@page import="com.landray.kmss.tib.sap.sync.model.ClocalVo"%>
...
<p class="txttitle">
	<bean:message bundle="tib-sap-sync" key="table.tibSapSyncJob" />
</p>
...
function edition_LoadIframe(index,tableName,dbID,tempId){
	...
	/*if(iframe.src==""){
		<%-- 传值不允许中文  --%>
		src=Com_Parameter.ContextPath+"tib/sap/sync/tib_sap_sync_temp_func/tibSapSyncTempFunc.do?method=listDStable&tableName="+tableName+"&dbID="+dbID+"&tempId="+tempId;
		iframe.src = src;
	}*/

	if($(iframe).attr("src").length<1){
	    var src=Com_Parameter.ContextPath+"tib/sap/sync/tib_sap_sync_temp_func/tibSapSyncTempFunc.do?method=listDStable&tableName="+tableName+"&dbID="+dbID+"&tempId="+tempId;
		$(iframe).attr("src",src);
	}
}
