	...
	<template:replace name="path">
		<ui:combin ref="menu.path.tib.simplecategory">
			<ui:varParams 
				...
				cateTitle="${ lfn:message('tib-sap-sync:tib.sap.sync') }" 
				href="javascript:parent.tibLoadList('/tib/sap/sync/tib_sap_sync_job/tibSapSyncJob_ui_include.jsp?categoryId=!{value}');" 
	...
	<template:replace name="content">
		...
		<list:criteria>
			...
			<list:cri-ref ref="criterion.sys.simpleCategory" key="docCategory" multi="false" title="分类导航" expand="true">
			  <list:varParams modelName="com.landray.kmss.tib.sap.sync.model.TibSapSyncCategory"/>
			</list:cri-ref>
			<list:cri-auto modelName="com.landray.kmss.tib.sap.sync.model.TibSapSyncJob" property="fdEnabled"/>
		</list:criteria>
					...
					<td align="right">
						<ui:toolbar>
							...
							<kmss:auth requestURL="/tib/sap/sync/tib_sap_sync_job/tibSapSyncJob.do?method=deleteall">
								<ui:button text="${lfn:message('button.delete')}" order="4" onclick="delDoc('${LUI_ContextPath}/tib/sap/sync/tib_sap_sync_job/tibSapSyncJob.do?method=deleteall')"></ui:button>
							</kmss:auth>
							<kmss:auth requestURL="/tib/sap/sync/tib_sap_sync_job/tibSapSyncJob.do?method=chgenabled">
								<ui:button text="${lfn:message('sys-quartz:sysQuartzJob.button.enable')}"
									onclick="tibEnableSave('${LUI_ContextPath}/tib/sap/sync/tib_sap_sync_job/tibSapSyncJob.do?method=chgenabled&fdEnabled=true');"></ui:button>
							</kmss:auth>	
							<kmss:auth requestURL="/tib/sap/sync/tib_sap_sync_job/tibSapSyncJob.do?method=chgenabled">
								<ui:button text="${lfn:message('sys-quartz:sysQuartzJob.button.disable')}"
									onclick="tibEnableSave('${LUI_ContextPath}/tib/sap/sync/tib_sap_sync_job/tibSapSyncJob.do?method=chgenabled&fdEnabled=false');"></ui:button>
							</kmss:auth>
						</ui:toolbar>	
					</td>
				</tr>
		...
		<list:listview id="listview">
			<ui:source type="AjaxJson">
				{url:'/tib/sap/sync/tib_sap_sync_job/tibSapSyncJobIndex.do?method=list&categoryId=${param.categoryId }'}
			</ui:source>
			<%--列表形式--%>
			<list:colTable isDefault="false" layout="sys.ui.listview.columntable" 
				rowHref="/tib/sap/sync/tib_sap_sync_job/tibSapSyncJob.do?method=view&fdId=!{fdId}" name="columntable">
		...
		seajs.use(['lui/jquery', 'lui/util/str', 'lui/dialog','lui/topic'], function($, strutil, dialog , topic) {
			...
			window.addDocByCate = function(){
				parent.addDoc('${LUI_ContextPath}/tib/sap/sync/tib_sap_sync_job/tibSapSyncJob.do?method=add&fdtemplatId='+ cateId);
			};
			//根据筛选器分类异步校验权限
