	...
	<tr>
		...
		<table id="List_ViewTable1" class="tb_noborder">
			...
			<tr>
				...
				<td align="left">在函数管理中配置抽取需要的BAPI函数</td>
			</tr>
			<tr>
				...
				<td align="left">在当前页面配置定时任务：<br>
				  a)选择函数<br>
				   b)选择数据源<br>
				    c)若SAP中的数据和RDB有关联，则需要先选择数据库表，<br>再在映射字段中选择数据库表的列名
				</td>
			</tr>
		...
		<table id="List_ViewTable2" class="tb_noborder">
			...
			<tr>
				...
				<td  align="left">指执行定时任务时，传递到SAP中的数据</td>
			</tr>
			<tr>
				<td align="center">2</td>
				<td align="left">传出参数</td>
				<td  align="left">指执行定时任务时，从SAP返回的数据</td>
			</tr>
			<tr>
				<td align="center">3</td>
				<td align="left">传入表格</td>
				<td  align="left">指执行定时任务时，若是SAP映射了数据库表，则将数据库表中的数据全部传输到SAP中的表参数中</td>
			</tr>
			<tr>
				<td align="center">4</td>
				<td align="left">传出表格</td>
				<td  align="left">指执行定时任务时，将SAP中的表参数中的数据全部返回到对应的RDB中</td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td  align="left">在所选的数据库表中列名，与SAP字段对应</td>
			</tr>
			<tr>
				<td align="center">7</td>
				<td align="left">固定值</td>
				<td  align="left">仅在传入参数或者传入表格中存在，可将配置的固定值传输到SAP中</td>
			</tr>
			<tr>
				<td align="center">8</td>
				<td align="left">最后执行时间</td>
				<td  align="left">仅在传入参数或者传入表格中存在，将当前定时任务上次执行的时间传输到SAP中，若是第一次执行则传空值</td>
			</tr>
			<tr>
				<td align="center">9</td>
				<td align="left">最后更新时间</td>
				<td  align="left">将定时任务执行时的当前服务器时间传输到SAP中。</td>
			</tr>
			<tr>
		...
		<table id="List_ViewTable3" class="tb_noborder">
			...
			<tr>
				<td width="7%" align="center">1</td>
				<td align="left">【全量】</td>
				<td align="left">不需要选择主键或者KEY。每次同步时，会将RDB中的数据全部删除，再将SAP的数据全部插入</td>
			</tr>
			<tr>
				<td align="center">2</td>
				<td align="left">【增量】</td>
				<td align="left">需要选择至少一个主键。每次同步时，系统根据主键将SAP和RDB的数据进行比对，若RDB中无数据，则插入，有则更新。</td>
			</tr>
			<tr>
				<td align="center">3</td>
				<td align="left">【增量（插入前删除）】</td>
				<td align="left">需要至少选择一个KEY。每次同步时，系统会遍历SAP传输过来的数据，以KEY去RDB中寻找对应数据，并全部删除。<br>系统再将SAP传入过来的数据全部插入到RDB中。</td>
			</tr>
			<tr>
				<td align="center">4</td>
				<td align="left">【增量（时间戳）】</td>
				<td align="left">需要选择时间戳列（来自SAP字段）和至少一个主键。每次同步时，系统会将时间戳列于定时任务最后执行时间作比对，<br>如该数据的时间戳列>定时任务最后执行时间，则将数据按照【增量】的方式同步到RDB中。</td>
			</tr>
			<tr>
				<td align="center">5</td>
				<td align="left">【增量（条件删除】</td>
				<td align="left">需要配置条件公式和选择至少一个KEY。每次同步时，系统根据配置的条件公式去判断SAP传输出来的数据，若满足，<br>则该数据为需要删除的数据。系统用该数据的KEY去RDB中寻找并删除。然后再将SAP中传输出来的不<br>满足条件的数据，以【增量】的方式同步到RDB中。</td>
			</tr>
			<tr>
				<td align="center">6</td>
				<td align="left">【<img alt="" src="${KMSS_Parameter_StylePath}calendar/finish.gif"> 一键匹配】</td>
				<td align="left">一种便捷配置方式。系统将SAP的字段名称和数据库表列名作比对，若相等，则帮助用户自动选择右边的数据库表列。</td>
			</tr>
