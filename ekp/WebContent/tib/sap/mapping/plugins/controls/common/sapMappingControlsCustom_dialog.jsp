...
<table cellpadding=0 cellspacing=0 style="height:100%; border-collapse:collapse;border: 0px #303030 solid;">
	<tr>
		<td valign="top" style="border:#303030 solid; border-width:0px 1px 0px 0px; border-collapse:collapse;">
			<iframe width=200 height=100% frameborder=0 scrolling=auto src='../common/sapMappingControlsDialogCustom_tree.jsp'></iframe>
		</td>
		<td width="10px">&nbsp;</td>
		<td valign="center">
...
<table class="tb_normal" width="100%">
	<tr width="100%">
		<td width="100%">
			参数是否必须：
			<input type="radio" name="isRequired" value="1" checked/>是
			<input type="radio" name="isRequired" value="0" />否
		...
		<td align=center>
			<br>
				...
				<input type="button" value="<bean:message bundle="sys-formula" key="button.help"/>" 
					onClick="Com_OpenWindow('../common/sapMappingControlsDialog_help.jsp', '_blank');">
		</td>
	</tr>
</table>		
		</td>
...
<script type="text/javascript">
   ...
   function validateFormula_self(action){
	   ...
	   if(validateResult!=null){
		...
		if (idField == "") {
			idField = dialogObject.formulaParameter.tibEkpId;
		}
		var isRequired = "";
		for (var i = 0, len = isRequiredObjs.length; i < len; i++) {
			if (isRequiredObjs[i].checked == true) {
				isRequired = isRequiredObjs[i].value;
			}
		}
		...
		//转换表达式
		var scriptIn = replaceSymbol(idField.replace(/\\$/g, "") + "|" + expressionValue + "|" + isRequired);
		...
		validateResult = {name:scriptIn, id:scriptIn};
    }

	// 编辑选中
    var textValue = "${param.isRequired}";
    if (parseInt(textValue) == 1) {
    	document.getElementsByName("isRequired")[0].checked = true;
    } else if(parseInt(textValue) == 0) {
    	document.getElementsByName("isRequired")[1].checked = true;
    }
