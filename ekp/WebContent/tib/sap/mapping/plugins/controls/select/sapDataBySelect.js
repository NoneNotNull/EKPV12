...
Designer_Config.controls['sapDataBySelect'] = {
		type : "sapDataBySelect",
		storeType : 'field',
		inherit : 'base',
		onDraw : _Designer_Control_SapDataBySelect_OnDraw, //绘制函数
		drawXML : _Designer_Control_SapDataBySelect_DrawXML, //生成数据字典
		...
		attrs : { // 这个是对话框属性配置
			...
			dataSource : {
			    ...
			    required: true,
			    show: true,
			    draw: _Designer_Control_SapDataBySelect_DataSource_Self_Draw,
			    checkout: Designer_SapDataBySelect_Control_Attr_Required_Checkout
			...
			selectShowValue : {
			    ...
			    required: true,
			    show: true,
			    draw: _Designer_Control_SapDataBySelect_SelectShowValue_Self_Draw,
			    checkout: Designer_SelectShowValue_Control_Attr_Required_Checkout,
			...
			selectActualValue : {
				...
				required: true,
				show: true,
			    draw: _Designer_Control_SapDataBySelect_SelectActualValue_Self_Draw,
				checkout: Designer_SelectActualValue_Control_Attr_Required_Checkout,
				convertor: Designer_Control_Attr_HtmlQuotEscapeConvertor
			},
			inputParam : {
				...
				required: false,
				show: true,
				draw: _Designer_Control_SapDataBySelect_InputParam_Self_Draw
			},

			formula: Designer_Config.attrs.formula,
			reCalculate: Designer_Config.attrs.reCalculate
		},
		onAttrLoad : _Designer_Control_Attr_SapDataBySelect_OnAttrLoad,
		info : {
			name: SapControls_Lang.sapDataBySelect
		},
		resizeMode : 'onlyWidth'
};

Designer_Config.operations['sapDataBySelect'] = {
		imgIndex : 18,
		title : SapControls_Lang.sapDataBySelect,
		run : function (designer) {
			designer.toolBar.selectButton('sapDataBySelect');
		},
		type : 'cmd',
		shortcut : 'X',
		sampleImg : 'style/img/select.jpg',
		select: true,
		cursorImg: 'style/cursor/getSelect_esap.ico'
};
//把select选择控件增加到控件区
Designer_Config.buttons.control.push("sapDataBySelect");
//把select选择控件增加到右击菜单区
Designer_Menus.add.menu['sapDataBySelect'] = Designer_Config.operations['sapDataBySelect'];
 ...
 * @param control
 */ 
function _Designer_Control_Attr_SapDataBySelect_OnAttrLoad(form,control){	
	// 设置XML和明细表
	_SelectGetTemplateXmlAndSetJson(form._inputParamJson);
};

//获取XML模版
function _SelectGetTemplateXmlAndSetJson(formTableJson) {
	...
	data.SendToBean("tibSapMappingXmlTemplateBean&rfcId="+ rfcId +"&type=func", 
			function(rtnData){_Select_Edit_Control_CallXml(rtnData, formTableJson);});
}

function _Select_Edit_Control_CallXml(rtnData, formTableJson) {
	SapDataSelect_FuncXml = rtnData.GetHashMapArray()[0]["funcXml"];
...
//		alert(formTableJson.value);
		if(inputParamsJson.TABLE_DocList != null) {
			_Select_Edit_Control_AppendRow(inputParamsJson.TABLE_DocList);
		}
	}
}

// 编辑时初始化
function _Select_Edit_Control_AppendRow(tableJson) {
	// 删除明细表行
	SapDataBySelect_DeleteTableByMxId("TABLE_DocList");
	for (var i = 0, lenI = tableJson.length; i < lenI; i++) {
		...
		// 下拉选择框SAP
		var xmlObj = XML_CreateByContent(SapDataSelect_FuncXml);
...
function SapDataBySelect_DeleteTableByMxId(mxId) {
 ...
 * @param childNode
 */ 
function _Designer_Control_SapDataBySelect_OnDraw(parentNode, childNode){
	...
	var htmlCode = _Designer_Control_SapDataBySelect_DrawByType(this.parent, this.attrs, this.options.values, this);
 ...
 * 生成数据字典
 */
function _Designer_Control_SapDataBySelect_DrawXML() {	
	...
	buf.push('name="', values.id+'_value', '" ');
	buf.push('label="', values.label, '" ');
	buf.push('type="String" ');
	if (values.required == "true") {
		buf.push('notNull="true" ');
...
*/
function GetInputParameterJsonByTable(id) {
	...
	for (var i = 1, length = table.rows.length; i < length; i++) {
		rtnJson.push(GetInputParamterJsonByRow(id, table.rows[i]));
	}
...
根据table的id 保存每行的数据
*/
function GetInputParamterJsonByRow(id, trElement) {
	var rtnJson = new Array();
	rtnJson.push("{");
	// 是否必填
	var isRequired = GetElementsByParent(trElement.cells[0], "input")[0].checked ? "true" : "false";
	rtnJson.push("'inputParamRequired':'" + isRequired + "'");
	// SAP
	var selectObjs = GetElementsByParent(trElement.cells[1], "select");
 ...
 * @returns {String}
 */ 
function _Designer_Control_SapDataBySelect_DrawByType(parent, attrs, values, control) {
	var htmlCode = '<input type="hidden" id="'+values.id+'"';
	...
	// 数据来源
	if(values._dataSource !=null ||  values._dataSource!=''){
      	htmlCode += " _dataSource=\""+values._dataSource+"\" ";
	} 
	// 函数ID
	...
	// 弹出框显示值
	if (values.selectShowValue != null || values.selectShowValue != '') {
		htmlCode += " selectShowValue='"+ values.selectShowValue +"'";
	}
	
	// 弹出框实际值
	if (values.selectActualValue != null || values.selectActualValue != '') {
		htmlCode += " selectActualValue='"+ values.selectActualValue +"'";
	}
	...
	if(values._inputParamJson!=null || values._inputParamJson!=''){		
		...
		rtnJson.push("{");
		// 输入参数
		rtnJson.push(GetInputParameterJsonByTable("TABLE_DocList"));
		// 结束
		rtnJson.push("}");
	...
	if (attrs.businessType) {
		...
		if ((values.businessType == 'dateDialog' ||  values.businessType == 'timeDialog' ||  values.businessType == 'datetimeSelect')) {
	...
	htmlCode += ' readOnly >';
	htmlCode += '<select><option>'+ SapControls_Lang.select +'</option>';
	htmlCode += '</select>';
 ...
 * @returns {Boolean}
 */ 
function Designer_SapDataBySelect_Control_Attr_Required_Checkout(msg, name, attr, value, values, control) {
	if (values._dataSource ==null || values._dataSource == "" || values._dataSource == "") {
...
function Designer_SelectShowValue_Control_Attr_Required_Checkout(msg, name, attr, value, values, control) {
	if (values.selectShowValue ==null || values.selectShowValue == "") {
...
function Designer_SelectActualValue_Control_Attr_Required_Checkout(msg, name, attr, value, values, control) {
	if (values.selectActualValue ==null || values.selectActualValue == "" || values.selectActualValue == "") {
...
// 数据来源   自绘函数
function _Designer_Control_SapDataBySelect_DataSource_Self_Draw(name, attr, value, form, attrs, values) {
	// 实际数据来源名称
	var html = "<input type='text' name='_dataSource'  class='inputsgl'  id='_dataSource' readonly  style='width: 86%'";	
	html += " value=\"" + (values._dataSource != "undefined" && values._dataSource != null ? values._dataSource : "") + "\">";	
	// 隐藏ID
	...
	html += "<img src='"+ Com_Parameter.StylePath +"icons/edit.gif' alt='"+ SapControls_Lang.chooseFunc 
			+"' onclick='DoSelect_TreeList_DataSource(this)' style='cursor: hand'>";
	return Designer_AttrPanel.wrapTitle(name, attr, value, html);
};

function DoSelect_TreeList_DataSource(dom){
	var rfcId = document.getElementById("fdRfcSettingId").value;
	// 显示确定按钮
	Designer_AttrPanel.showButtons(document.getElementById("_dataSource"));
	Dialog_TreeList(false, 'fdRfcSettingId', '_dataSource', ';', 
...
function Call_DataSource(rtnData) {
	if (rtnData == undefined || rtnData == "") {
		return;
	}
	// 重新选择数据来源后，要把全局的XML模版定义为Null
	SapDataSelect_FuncXml = null;
	var rfcId = rtnData.GetHashMapArray()[0]["id"];
	var data = new KMSSData();
	data.SendToBean("tibSapMappingXmlTemplateBean&rfcId="+ rfcId, Call_FuncXml);
}

// 全局存放的XML模版赋值
var SapDataSelect_FuncXml;
function Call_FuncXml(rtnData) {
	SapDataSelect_FuncXml = rtnData.GetHashMapArray()[0]["funcXml"];
}

// 显示值   自绘函数
function _Designer_Control_SapDataBySelect_SelectShowValue_Self_Draw(name, attr, value, form, attrs, values) {
	var html = "<input type='text' name='selectShowValue'  class='inputsgl'  id='selectShowValue' readonly  style='width: 86%'";	
	html += " value='" + (values.selectShowValue != "undefined" && values.selectShowValue != null ? values.selectShowValue : "") + "'>";	
	html += "<img src='"+ Com_Parameter.StylePath +"icons/edit.gif' alt='"+ SapControls_Lang.chooseField 
			+"' onclick=\"DoSelect_TreeList_Common('selectShowValue', 'selectShowValue')\" style='cursor: hand'>";
	return Designer_AttrPanel.wrapTitle(name, attr, value, html);
};


//实际值   自绘函数
function _Designer_Control_SapDataBySelect_SelectActualValue_Self_Draw(name, attr, value, form, attrs, values) {
	var html = "<input type='text' name='selectActualValue'  class='inputsgl'  id='selectActualValue' readonly  style='width: 86%'";	
	html += " value='" + (values.selectActualValue != "undefined" && values.selectActualValue != null ? values.selectActualValue : "") + "' />";	
	html += "<img src='"+ Com_Parameter.StylePath +"icons/edit.gif' alt='"+ SapControls_Lang.chooseField 
	+"' onclick=\"DoSelect_TreeList_Common('selectActualValue', 'selectActualValue')\" style='cursor: hand'>";
	return Designer_AttrPanel.wrapTitle(name, attr, value, html);
};
 ...
 */
function DoSelect_TreeList_Common(bindId, bindName) {
	// 如果xml为null,则弹出正在加载XML模版或者未加载
	if (!Checked_DataSource()) {
		return;
	}
	// 显示确定按钮
	Designer_AttrPanel.showButtons(document.getElementById("selectActualValue"));
	// XML字符串转为XML对象
	var funcXmlObj = XML_CreateByContent(SapDataSelect_FuncXml);
	var xpath = "/jco/export/structure/field|/jco/tables/table[@rows='0']/records/field";
	var fieldInfo = _GetFieldValues(funcXmlObj, xpath);
	var varInfo = _ParseVarInfo(fieldInfo, "name", "val");
...
// 解析为Json
function _ParseNode4Json (node, parentXpath, index, defAttrs) {
	var nodeInfo = {};
	if (!node)
		return null;
	...
	nodeInfo.xpath = _AddXpath(parentXpath, node.nodeName, index);
...
function _AddXpath(xpath, nodeName, index) {
	if (index || index == 0) {
 ...
 * @param {}    varInfo
 */
function _ParseVarInfo(varInfo, text, value) {
	var parseRtn = [];
	if (varInfo) {
  ...
  * @param {}    varInfo
  */
 function _ParseVarInputParamInfo(varInfo) {
 	var parseRtn = [];
 	if (varInfo) {
 		for ( var i = 0, len = varInfo.length; i < len; i++) {
 			parseRtn.push( {
 ...
 * @return
 */
function Checked_DataSource() {
	var rfcId = document.getElementById("fdRfcSettingId").value;
	if (rfcId == "") {
		alert(SapControls_Lang.pleaseChooseDataSource);
		return false;
	}
	if (SapDataSelect_FuncXml == null) {
 ...
 * 传入参数  自绘函数
 */
function _Designer_Control_SapDataBySelect_InputParam_Self_Draw(name, attr, value, form, attrs, values) {
 	...
 	html += "<img src='"+ Com_Parameter.StylePath +"icons/add.gif' alt='"+ SapControls_Lang.add +"' "+
					"onclick='_AddInputParam();' style='cursor: hand'></span>";
 	html += "</td></tr>";
 	// 基准行
 	...
 	html += "onclick=\"DoSelect_TreeList_InputParam('_inputParamBandId!{index}', '_inputParamText!{index}', '!{index}','_inputParamId!{index}')\" style='cursor: hand'>";
 	html += "<img src='"+ Com_Parameter.StylePath +"icons/delete.gif' alt='del'";
 	html += "onclick=\"Designer_AttrPanel.showButtons(document.getElementById('_inputParamBandId!{index}'));Select_DocList_DeleteRow();\" style='cursor: hand'>";
 	html += "</td></tr>";
 	html += "</table>";
...
function DoSelect_TreeList_InputParam(bindId, bindName, index, ekpId) {
	// 把是否必填的值带过去
	...
	var varInfo = _ParseVarInputParamInfo(Designer.instance.getObj(true));
	var dialog = new KMSSDialog();
	dialog.SetAfterShow(function(rtnData){Call_InputParamSelect(rtnData, index);});
	dialog.formulaParameter = {
		varInfo : varInfo,
		returnType : "Object",
...
// 特殊公式定义器后回调
function Call_InputParamSelect(rtnData, index) {
...
***********************************************/
function Select_DocList_DeleteRow(optTR){
	if(optTR==null)
	...
	for(var i = rowIndex; i<tbInfo.lastIndex; i++)
		Select_DocListFunc_RefreshIndex(tbInfo, i);
}

function Select_DocListFunc_RefreshIndex(tbInfo, index){
	for (var n = 0; n < tbInfo.cells.length; n ++) {
		if (tbInfo.cells[n].isIndex) {
			tbInfo.DOMElement.rows[index].cells[n].innerHTML = index;
		}
	}
	Select_DocListFunc_RefreshFieldIndex(tbInfo, index, "INPUT");
	Select_DocListFunc_RefreshFieldIndex(tbInfo, index, "SELECT");
	Select_DocListFunc_RefreshFieldIndex(tbInfo, index, "IMG");
}

function Select_DocListFunc_RefreshFieldIndex(tbInfo, index, tagName){
