...
<!--
 $(document).ready(function(){
	 ...
	 $("#del_button").click(function(){
	  	  if(!window.confirm("<bean:message key='kmCollaborate.jsp.confirm.delete.seleteData' bundle='km-collaborate' />"))
	  		   return;
	...
	<table id="List_ViewTable">
		...
		<c:forEach items="${queryPage.list}" var="kmCollaborateMain" varStatus="vstatus">
			<tr
				...
				<c:if test="${param.status eq 10 }">kmss_href="<c:url value="/km/collaborate/km_collaborate_main/kmCollaborateMain.do" />?method=edit&fdId=${kmCollaborateMain.fdId}"</c:if>
