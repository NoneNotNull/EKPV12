...
<template:include ref="default.list">
   ...
   <template:replace name="path"> 
		<ui:menu layout="sys.ui.menu.nav"  id="categoryId"> 
			...
			<ui:menu-item text="收文管理" href="/km/imissive/km_imissive_receive_main/index.jsp" target="_self">
	      ...
	      <ui:accordionpanel>
		        <ui:combin ref="menu.nav.favorite.category">
						<ui:varParams 
							...
							onClick="LUI('receiveCriteria').setValue('fdTemplate','!{value}');"/>
				</ui:combin>
	...
	<template:replace name="content">  
	  <list:criteria id="receiveCriteria">
	      ...
	      <list:cri-criterion title="${ lfn:message('km-imissive:kmImissiveReceiveMain.Receive.my') }" key="mydoc" multi="false">
				<list:box-select>
					<list:item-select>
			...
			<list:cri-auto modelName="com.landray.kmss.km.imissive.model.KmImissiveReceiveMain" property="fdReceiveTime;fdSigner;fdSignTime;fdIsFiling" />
	  </list:criteria>
			...
			<div style='color: #979797;float: left;padding-top:1px;'> 
					...
					<div style="float:left">
						...
						<ui:toolbar layout="sys.ui.toolbar.sort" style="float:left" >
						    ...
						    <list:sort property="fdDocNum" text="收文编号" group="sort.list"></list:sort>
						</ui:toolbar>
						</div>
					</div>
					<div style="float:left;">	
					...
					<div style="float:right">
						...
						<ui:toolbar count="3" id="Btntoolbar">
							...
							<kmss:auth requestURL="/km/imissive/km_imissive_receive_main/kmImissiveReceiveMain.do?method=deleteall&categoryId=${param.categoryId}&nodeType=${param.nodeType}" requestMethod="GET">
								<ui:button id='del' text="${lfn:message('button.delete')}" onclick="delDoc()" order="3">
								</ui:button>
							</kmss:auth>
							<%-- 修改权限 --%>
		...
		<list:listview id="listview">
			<ui:source type="AjaxJson">
					{url:'/km/imissive/km_imissive_receive_main/kmImissiveReceiveMain.do?method=listChildren&categoryId=${param.categoryId}'}
			</ui:source>
	 	...
	 	<script type="text/javascript">
			...
			seajs.use(['lui/jquery','lui/dialog','lui/topic','lui/toolbar'], function($, dialog , topic,toolbar) {
				...
				window.chgSelect = function() {
					...
					Com_OpenWindow('<c:url value="/km/imissive/km_imissive_receive_main/kmImissiveChangeTemplate.jsp" />?values='+values+'&categoryId=${param.categoryId}');
					return ;
				};
				//归档
				...
				//批量签收
				window.updateSignAllDoc = function(){
					var values = [];
					$("input[name='List_Selected']:checked").each(function(){
							values.push($(this).val());
						});
					...
					dialog.confirm('<bean:message bundle="km-imissive" key="alert.msg"/>',function(value){
						if(value==true){
							window.del_load = dialog.loading();
							$.post('<c:url value="/km/imissive/km_imissive_receive_main/kmImissiveReceiveMain.do?method=updateSignAll"/>',
									$.param({"List_Selected":values},true),updateSignAllCallback,'json');
						}
					});
				};
				window.updateSignAllCallback = function(data){
					if(window.del_load!=null)
						window.del_load.hide();
					if(data!=null && data.status==true){
						topic.publish("list.refresh");
