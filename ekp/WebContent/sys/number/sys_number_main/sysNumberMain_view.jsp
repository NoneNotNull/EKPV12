	...
	<c:if test="${param['isCustom'] !='1'}">
		<div id="optBarDiv">
				<input type="button"
					...
					onclick="Com_OpenWindow('sysNumberMain.do?method=edit&isSysnumberAdmin=${param.isSysnumberAdmin}&fdId=${param.fdId}&modelName=${param.modelName}','_self');">
				<input type="button"
					value="<bean:message key="button.delete"/>"
					onclick="if(!confirmDelete())return;Com_OpenWindow('sysNumberMain.do?method=delete&isSysnumberAdmin=${param.isSysnumberAdmin}&fdId=${param.fdId}&modelName=${param.modelName}','_self');">
				<input type="button"
	...
	<c:if test="${param['isCustom']!='1'}">
		<tr>
			...
			<td class="td_normal_title" width=15%>
				...
				<input type="checkbox" name="DefaultFlag" value="0" onclick="return false"/>
	...
	<tr>
		<td width=100% colspan="4">
			<object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" width="800px" height="250px" id="IDRuleMaker"  name="IDRuleMaker">
                ...
                <param name="flashvars" value='view=true&langParam={"fdTitle":"<bean:message bundle="sys-number" key="sysNumberMain.fdTitle"/>","fdDate":"<bean:message bundle="sys-number" key="sysNumberMain.fdDate"/>","fdTime":"<bean:message bundle="sys-number" key="sysNumberMain.fdTime"/>","flowNo":"<bean:message bundle="sys-number" key="sysNumberMain.flowNo"/>","fdConstant":"<bean:message bundle="sys-number" key="sysNumberMain.fdConstant"/>","fdCustom":"<bean:message bundle="sys-number" key="sysNumberMain.fdCustom"/>","fdClear":"<bean:message bundle="sys-number" key="sysNumberMain.fdClear"/>"}' />
                <embed
					name="IDRuleMaker" 
					src="IDRuleMaker.swf"
					wmode="opaque"
					...
					flashvars='view=true&langParam={"fdTitle":"<bean:message bundle="sys-number" key="sysNumberMain.fdTitle"/>","fdDate":"<bean:message bundle="sys-number" key="sysNumberMain.fdDate"/>","fdTime":"<bean:message bundle="sys-number" key="sysNumberMain.fdTime"/>","flowNo":"<bean:message bundle="sys-number" key="sysNumberMain.flowNo"/>","fdConstant":"<bean:message bundle="sys-number" key="sysNumberMain.fdConstant"/>","fdCustom":"<bean:message bundle="sys-number" key="sysNumberMain.fdCustom"/>","fdClear":"<bean:message bundle="sys-number" key="sysNumberMain.fdClear"/>"}'>
				</embed>
            </object>
		</td>
	</tr>
			...
			<fieldset>
				...
				<tr >
					<td width="20%" align="center">
				 	...
				 	<td> 
				 		<input type="hidden" id="textCustomID" name="textCustomID"/>
				 		<input type="text" id="textCustomName" name="textCustomName" readonly size="40"  class="inputsgl"/>
		 ...
		 if(t_type=='date'){
			...
			$("#divTime").toggle(300);
			 ...
			 $.each(t_ruleData,function(idx,item){
				 ...
				 if(idx=='formulaID')
					 $("#textCustomID").val(item);
				 else if(idx=='formulaName')
					 $("#textCustomName").val(item);
				});
...
function divFlowOK(){
	...
	if(period==undefined)
		period=0;
	var start=$.trim($("#start").attr("value"));
	var len=$.trim($("#len").attr("value"));
	var step=$.trim($("#step").attr("value"));
	var areas_json="";
...
function divConstOK(){
	var textConst=$("#textConst").attr("value");
	var dateFormatJson="{\"id\":\""+json_id+"\",\"type\":\""+json_type+
...
function divCustomOK(){
	var textCustomID=$("#textCustomID").attr("value");
	var textCustomName=$("#textCustomName").attr("value");
	var textCustomNameFromUser=$("#textCustomNameFromUser").attr("value");
